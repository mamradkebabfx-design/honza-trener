...
- `index.html` — single-page layout, restructured around a Hormozzi-style copywriting flow (hook → problem/agitate → offer → proof → objections → CTA). Current section order: hero (`#top`) → problem/for-whom (`.forwhom`, no id) → offer/value-stack for the free consultation (`.offer`, `#konzultace`) → about/proof (`#o-mne`) → benefits (`#benefity`, 4 cards) → how-it-works steps (`.steps`, `#jak-to-funguje`) → FAQ (`#faq`) → CTA/contact (`#kontakt`) → footer. The primary (and currently only) CTA throughout is a **free intro consultation** ("Zarezervovat konzultaci zdarma") — there is no pricing section on the live page (see Conventions section below for why, and the git tag that preserves the version with pricing). Contains inline JSON-LD structured data (LocalBusiness, Person, Service+Offer with just a 0 Kč "Úvodní konzultace" offer, FAQPage) — keep it in sync with visible copy when editing pricing, FAQ, or contact info. Footer links to `privacy-policy.html`.
...
- `assets/hero-video.mp4` — autoplay/muted/loop background video shown in a rounded, floating card on the right side of the hero (`.hero__media`/`.hero__bg`; `.hero__bg` is a `<video>`, not an `<img>`). The card's aspect-ratio is pinned to `9/16` to match the video exactly (no cropping, no letterboxing). On mobile (`≤1024px`) the card drops out of absolute positioning and stacks below the hero text in normal flow. Sourced from a 79 MB HEVC/4K portrait export on the user's Desktop — re-encoded with `ffmpeg` to H.264, 1080×1920, no audio track, `-movflags +faststart`, ~4.4 MB. **If the video is ever replaced, re-encode the same way** (`ffmpeg -i <source> -vf "scale=1080:-2" -r 25 -an -c:v libx264 -profile:v high -preset slow -crf 26 -pix_fmt yuv420p -movflags +faststart hero-video.mp4`) — an unprocessed HEVC/large source will not play reliably in Chrome/Firefox and will bloat the hero's load time.
- `assets/hero-video-wide.mp4` — an earlier full-bleed variant (blurred/scaled copy of the same footage filling a 1920×960 canvas behind the sharp center video) built while iterating on the hero design. **Not referenced by any HTML/CSS** — the final direction went with the boxed-card layout above instead. Left on disk (untracked in git) in case the full-bleed treatment is revisited; safe to delete otherwise.
