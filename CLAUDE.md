...
- `index.html` — single-page layout, restructured around a Hormozzi-style copywriting flow (hook → problem/agitate → offer → proof → objections → CTA). Current section order: hero (`#top`) → problem/for-whom (`.forwhom`, no id) → offer/value-stack for the free consultation (`.offer`, `#konzultace`) → about/proof (`#o-mne`) → benefits (`#benefity`, 4 cards) → how-it-works steps (`.steps`, `#jak-to-funguje`) → FAQ (`#faq`) → pricing (`#cenik`) → CTA/contact (`#kontakt`) → footer. The primary CTA throughout is a **free intro consultation** ("Zarezervovat konzultaci zdarma"), not a paid package directly — pricing stays lower on the page for transparency. Contains inline JSON-LD structured data (LocalBusiness, Person, Service+Offer incl. a 0 Kč "Úvodní konzultace" offer, FAQPage) — keep it in sync with visible copy when editing pricing, FAQ, or contact info. Footer links to `privacy-policy.html`.
